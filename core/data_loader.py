...
def load_product_supply_chain_mappings() -> pd.DataFrame:
    ...
    if not path.exists():
        return _empty_dataframe(("source_company", "source_product", "direction", "related_company", "relation", "weight", "rationale", "updated_at"))
    df = pd.read_csv(path)
...
@lru_cache(maxsize=1)
def load_product_financials() -> pd.DataFrame:
    df = _read_csv("product_financials.csv")
    validate_dataframe("product_financials", df)
    return df


@lru_cache(maxsize=1)
def load_product_operating_metrics() -> pd.DataFrame:
    df = _read_csv("product_operating_metrics.csv")
    validate_dataframe("product_operating_metrics", df)
    return df


@lru_cache(maxsize=1)
def load_edges() -> pd.DataFrame:
    company_edges = load_company_relationships().rename(columns={"source_company": "source", "target_company": "target"})
    company_edges["source_type"] = "company"
    company_edges["target_type"] = "company"
    company_edges["source_dataset"] = "curated"

    company_product_edges = load_company_product_relationships().rename(columns={"company": "source", "product": "target"})
    ...
    product_edges = load_product_relationships().rename(columns={"source_product": "source", "target_product": "target"})
    product_edges["source_type"] = "product"
    product_edges["target_type"] = "product"
    product_edges["source_dataset"] = "curated"

    product_market_edges = load_product_market_relationships().rename(columns={"product": "source", "market": "target"})
...
def clear_data_caches() -> None:
    ...
    load_product_financials.cache_clear()
    load_product_operating_metrics.cache_clear()
